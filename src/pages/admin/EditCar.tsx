...
export default function EditCar() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const [brandsRes, categoriesRes, carRes] = await Promise.all([
        fetch(`${APP_URL}/brands`),
        fetch(`${APP_URL}/categories`),
        fetch(`${APP_URL}/cars/${id}`, {
      ...
      setFormData({
        brand_id: String(carData.brand_id),
        category_id: String(carData.category_id),
        model: carData.model,
        year: String(carData.year),
        price_per_day: String(carData.price_per_day),
        transmission: carData.transmission,
        fuel_type: carData.fuel_type,
        seats: String(carData.seats),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setError(data.message || "Error al actualizar el vehículo")
        setSubmitting(false)
        return
      }

      setSuccess("¡Vehículo actualizado correctamente!")
      setTimeout(() => navigate("/admin/manage-cars"), 1500)
    } catch {
    ...
    <>
      ...
      <div className="font-dm bg-neutral-950 min-h-screen">
        ...
        <header className="sticky top-0 z-10 bg-neutral-950/80 backdrop-blur-md border-b border-neutral-800 px-8 py-4">
          <h1 className="font-extrabold text-xl text-white">Editar vehículo</h1>
          <p className="text-neutral-500 text-xs mt-0.5">Modificá los datos del vehículo seleccionado</p>
        </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="number" step="0.01" name="price_per_day" value={formData.price_per_day}
              ...
              <div className="flex flex-wrap gap-2">
                {categories.map(c => (
                  <button
                    key={c.id}
                    type="button"
                    onClick={() => setFormData({ ...formData, category_id: String(c.id) })}
                    className={`px-4 py-2 rounded-full text-sm font-semibold border transition-all ${
                      formData.category_id === String(c.id)
          ...
          <section className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
            ...
            {imagePreview ? (
              ...
              <label className="group flex flex-col items-center justify-center border-2 border-dashed border-neutral-700 hover:border-blue-600 rounded-2xl p-12 text-center cursor-pointer transition-colors">
                ...
                <p className="font-semibold text-white text-sm mb-1">Reemplazar imagen</p>
                <p className="text-neutral-500 text-xs mb-4">Dejá vacío para mantener la imagen actual</p>
